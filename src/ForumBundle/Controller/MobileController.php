...
/////////////////////////////
    ...
    public function suppAction($id)
    {
        ...
        $e = new Sujet();
        $event=$this->getDoctrine()->getRepository('ForumBundle:Sujet')->find($id);
        $location=$this->getDoctrine()->getRepository('ForumBundle:CommentaireSu')->findBy(array('post_id'=>$id));
        $em = $this->getDoctrine()->getManager();
        if($location!=NULL){
            foreach ($location as $locatio ){
            $em->remove($locatio);
            $em->flush();
    }
    ...
    public function newcommAction(Request $request)
    {
        ...
        $sujet = $this->getDoctrine()->getRepository(Sujet::class)->find($request->get('id'));
        //$task->setId($request->get('id'));
        $task->setCommentSuContent($request->get('commentSuContent'));
        $task->setPostId($sujet);
